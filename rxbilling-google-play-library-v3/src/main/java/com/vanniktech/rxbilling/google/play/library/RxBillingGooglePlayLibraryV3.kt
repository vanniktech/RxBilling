...
@Suppress("Detekt.TooManyFunctions") class RxBillingGooglePlayLibraryV3 @JvmOverloads constructor(
        ...
        Observable.create<T> { emitter ->
          ...
          client.querySkuDetailsAsync(skuDetailsParams) { billingResult, skuDetailsList: List<SkuDetails>? ->
            if (billingResult.responseCode == BillingResponseCode.OK) {
              ...
              emitter.onError(RuntimeException("Querying failed with responseCode: $billingResult"))
            }
          }
        }
            ...
            emitter.setDisposable(purchaseSubject
                ...
                .subscribe({ (billingResponse, purchases) ->
                  when (billingResponse.responseCode) {
                    ...
                    else -> emitter.onError(PurchaseException(billingResponse.responseCode, billingResponse.debugMessage))
                  }
                }, emitter::onError))
          }
        }.subscribeOn(scheduler)
  ...
  @CheckReturnValue override fun getPurchasedInApps() = getPurchased(SkuType.INAPP) {
    PurchasedInApp.create(activity.packageName, it.sku, it.purchaseToken, DEFAULT_PURCHASE_STATE, it.purchaseTime)
  }

  @CheckReturnValue override fun getPurchasedSubscriptions() = getPurchased(SkuType.SUBS) {
    PurchasedSubscription.create(activity.packageName, it.sku, it.purchaseToken, DEFAULT_PURCHASE_STATE, it.purchaseTime)
  }
      ...
      client.startConnection(object : BillingClientStateListener {
        override fun onBillingSetupFinished(billingResult: BillingResult) {
          if (billingResult.responseCode == BillingResponseCode.OK) {
            ...
            logger.d("Could not connect to BillingClient. ResponseCode: $billingResult")
            billingClient = null
          }
        }
